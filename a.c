...
#define ON  1       //使能LED
#define OFF 0       //禁止LED
#define D 30        //30代表25厘米，检测超声波有效障碍物的距离 
#define J 250       //用于控制检测障碍物的角度 350大概是90度 
#define B 10         //后退时间   1s 
int flag=0;
...
void spin_left1(int,int);
void spin_right1(int,int);
void left1();
void right1();
/**
...
* @par History   无
*/
void spin_left1(int LeftSpeed, int RightSpeed)
{
  //左电机前进
...
* @par History   无
*/
void spin_right1(int LeftSpeed, int RightSpeed)
{
  //左电机前进
...
int main()
{
 ...
 while(1){
  ...
  if (LeftDistance < D && RightDistance < D && FrontDistance < D  ) //前方、右边、左边有路障，直接结束 
  {
  	//灯光七彩变换
  ...
  else if ( FrontDistance < D  && LeftDistance >= D) //当发现左侧有路，优先左转；原地左转
  {
    ///亮蓝色
  ...
  else if (LeftDistance < D && FrontDistance < D  ) //当发现只有右侧无障碍，原地右转
  {
    //亮品红色,向右转
...
}else if(flag==-1){//处于左侧第一轨道 ，只测量右边和前面 
  printf("%d", flag);   
  float distance;
  //定义舵机位置变量和小车前方,左侧,右侧距离
  int RightDistance = 0;   //右方距离值变量RightDistance
  ...
  if (RightDistance < D && FrontDistance < D  )
  {
   //亮品红色,后退
				corlor_led(ON, OFF, ON);
				back(B);	//倒退1.5秒 
				spin_right(J);//右转 
				run(100,100);	//右转后前进
				delay(60);   //移动两个跑道宽度距离  1s'
				car();        //循迹模块 
				flag=1;	
  }
  else if(FrontDistance < D && RightDistance >= D ){//前边障碍物就右转 
  	//亮品红色,向右转 
				corlor_led(ON, OFF, ON);
				spin_right(J);
				car();  //循迹模块 
  ...
  if (LeftDistance < D && FrontDistance < D  )
  {
    //亮品红色,后退
				corlor_led(ON, OFF, ON);
				back(B);	//倒退
				spin_left(J);
				run(100,100);	//右转后前进
				delay(1500);		//移动两个跑道宽度距离
				car();    //循迹模块 
				flag=-1; 
  }
  else if (LeftDistance >= D && FrontDistance < D) //当发现左侧距离大于右侧，原地左转
  {
    //亮蓝色
    corlor_led(OFF, OFF, ON);
    spin_left(J);
