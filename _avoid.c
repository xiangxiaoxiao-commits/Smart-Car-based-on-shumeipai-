...
void servo_color_carstate()
{
  ...
  switch(flag)
	{
		case -1:	//左跑道
			if (RightDistance < 30 && FrontDistance < 30  )		//右方和前方有障碍物
			{
				//亮品红色,后退
				...
				spin_right(250);
				run(100,100);	//右转后前进
				delay(100);		//移动两个跑道宽度距离
				flag=1;	
			}
			else if (RightDistance > 30 && FrontDistance < 30  )	//右方无障碍物，前方有
			{
				//亮品红色,向右转
				corlor_led(ON, OFF, ON);
				spin_right(250);
		...
		case 0:
			if (LeftDistance < 30 && RightDistance < 30 && FrontDistance < 30  )	//左右前方都有障碍物
			{
				brake();
				flag=0;
			}
			else if (LeftDistance > 30 && FrontDistance < 30  )	//左方无障碍物，前方有
			{
				//亮蓝色
			...
			else if (RightDistance > 30 && LeftDistance < 30 && FrontDistance < 30  )	//右方无障碍物，前方有
			{
				//亮品红色,后退
				corlor_led(ON, OFF, ON);
				back(100);	//倒退
		...
		case 1:		//右跑道
			if (LeftDistance < 30 && FrontDistance < 30  )		//左方和前方有障碍物
			{
				//亮品红色,后退
				corlor_led(ON, OFF, ON);
				back(100);	//倒退
